...
const sendOtp = asyncHandler(async (req, res) => {
  ...
  if (mobile && !/^[0-9]{10}$/.test(mobile)) {
    return sendError(res, { status: 400, message: "Enter a valid 10-digit mobile number." });
  }
  ...
  sendSuccess(res, {
    ...
    // Demo/dev mode: since no real email provider is configured yet, the
    // OTP is returned here so the flow can be fully tested. Remove this
    // field once a real email provider (SendGrid/SES/etc.) is wired into
...
const register = asyncHandler(async (req, res) => {
  ...
  const { user, token } = await authService.registerUser({ fullName, email, mobile, password });
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  if (!newPassword || !isValidPassword(newPassword)) {
    return sendError(res, { status: 400, message: "New password must be at least 8 characters, with a number and a special character." });
  }
...
const googleLogin = asyncHandler(async (req, res) => {
  ...
  if (!idToken) return sendError(res, { status: 400, message: "Missing Google credential." });

  try {
    const { user, token } = await authService.loginWithGoogle(idToken);
    setAuthCookie(res, token);
    sendSuccess(res, { message: "Logged in with Google.", data: { user: sanitizeUser(user), token } });
  } catch (err) {
    return sendError(res, { status: err.status || 400, message: err.message });
  }
});
