...
import { saveSelectedBusinessRequest, fetchProfile, toggleBookmarkRequest } from "../services/profileService";
...
function BusinessIdeas() {
    ...
    useEffect(() => {
        ...
        Promise.all([fetchMyEnrollments(), fetchMyCertificates(), fetchProfile()])
            .then(([e, c, progress]) => { setEnrollments(e); setCertificates(c); setBookmarkedIds(progress?.bookmarkedBusinessIds || []); })
        ...
        return (
            <MainLayout>
                <section className="listing-page">
                    ...
                    <Skeleton variant="cards" count={6} label="Loading your business ideas" />
                </section>
            </MainLayout>
        );
    }
        ...
        return (
            <MainLayout>
                <section className="listing-page">
                    <h1>Explore Business Ideas</h1>
                    <div className="login-required-note">
                        <p>Complete your skill assessment first to see business ideas matched to you.</p>
                        <button onClick={() => navigate("/skill-assessment")}>Take Skill Assessment</button>
                    </div>
                </section>
            </MainLayout>
        );
    ...
    const visibleBusinesses = q
        ? availableBusinesses.filter((b) => b.title.toLowerCase().includes(q) || b.description?.toLowerCase().includes(q))
        : availableBusinesses;
    ...
    return (
        <MainLayout>
            <section className="listing-page">
                ...
                <h1>Business Ideas For You</h1>
                <p>Based on your selected skill: <strong>{primarySkill}</strong></p>
                        ...
                        <div className="listing-grid">
                            {completedEnrollments.map((e) => (
                                <div key={e.id} className="listing-card completed-card">
                                    ...
                                    <button onClick={() => setReportCardFor(e)}>View Report Card</button>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {activeEnrollment ? (
                    <div className="listing-grid" style={{ marginTop: 30 }}>
                        <div className="listing-card active-card">
                            ...
                            <p>Finish this course (exam + interview) before starting another.</p>
                            <button onClick={() => navigate("/dashboard")}>Continue Your Journey</button>
                        </div>
                    </div>
                ) : availableBusinesses.length > 0 ? (
                    <>
                        <h2 style={{ marginTop: 30 }}>{completedEnrollments.length > 0 ? "Start Your Next Course" : "Pick a Course to Start"}</h2>
                                    ...
                                    <div key={business.id} className="listing-card">
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                            ...
                                            <button
                                                ...
                                                aria-label={bookmarkedIds.includes(String(business.id)) ? "Remove bookmark" : "Bookmark this idea"}
                                                title={bookmarkedIds.includes(String(business.id)) ? "Remove bookmark" : "Bookmark this idea"}
                                                style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: bookmarkedIds.includes(String(business.id)) ? "#F59E0B" : "#D1D5DB" }}
                                            >
                                        ...
                                        <div className="listing-meta">
                                            <span><strong>Investment:</strong> {business.investment}</span>
                                            <span><strong>Income:</strong> {business.income}</span>
                                        </div>
            ...
            {reportCardFor && (
                <ReportCard
                    ...
                    certificate={certificates.find((c) => c.businessTitle === reportCardFor.businessTitle)}
